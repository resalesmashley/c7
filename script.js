...
function openAccountModal(accountType) {
    ...
    accountModalState.type = accountType;

    configureAccountModal(accountType);
...
const ACCOUNT_ROLES = {
    parent: [
        { value: 'parent-primary', label: 'Primary Parent/Guardian' },
        { value: 'parent-secondary', label: 'Secondary Parent/Guardian' },
        { value: 'parent-caregiver', label: 'Caregiver' }
    ],
    teacher: [
        { value: 'teacher-volunteer', label: 'Sunday Volunteer' },
        { value: 'teacher-lead', label: 'Class Lead' },
        { value: 'teacher-support', label: 'Support Staff' }
    ],
    admin: [
        { value: 'admin-director', label: 'Ministry Director' },
        { value: 'admin-operations', label: 'Operations Lead' },
        { value: 'admin-security', label: 'Security & Safety' }
    ]
};

const ACCOUNT_SUCCESS_COPY = {
    parent: {
        title: 'Parent request received!',
        message: 'Thanks, {name}. We\'ll verify details and email login steps shortly.'
    },
    teacher: {
        title: 'Teacher request received!',
        message: 'Thanks, {name}. Our team will confirm your classroom assignment and follow up soon.'
    },
    admin: {
        title: 'Admin request received!',
        message: 'We\'ll confirm your elevated access and share the security checklist with you shortly, {name}.'
    }
};
...
function configureAccountModal(accountType) {
    const title = document.getElementById('account-modal-title');
    const subtitle = document.getElementById('account-modal-subtitle');
    const childWrapper = document.getElementById('account-child-wrapper');
    const classWrapper = document.getElementById('account-class-wrapper');
    const roleWrapper = document.getElementById('account-role-wrapper');
    ...
    const successView = document.getElementById('account-modal-success');

    if (!title || !subtitle || !childWrapper || !classWrapper || !roleWrapper || !classSelect || !roleSelect || !form || !successView) {
        return;
    }

    form.hidden = false;
    successView.hidden = true;
    clearAccountModalErrors();

    title.textContent = accountType === 'parent' ? 'Create Parent Account' 
        : accountType === 'teacher' ? 'Create Teacher Account'
        : 'Create Admin Account';

    subtitle.textContent = accountType === 'parent'
        ? 'Submit a request for a new parent or guardian to access the portal.'
        : accountType === 'teacher'
            ? 'Provide details so our team can provision your teaching credentials.'
            : 'Request elevated access for ministry leaders who manage secure data.';

    childWrapper.style.display = accountType === 'parent' ? 'block' : 'none';
    classWrapper.style.display = accountType === 'parent' || accountType === 'teacher' ? 'block' : 'none';

    if (accountType === 'parent') {
        classWrapper.querySelector('label').textContent = 'Classroom *';
    } else if (accountType === 'teacher') {
        classWrapper.querySelector('label').textContent = 'Primary Classroom *';
    }
    ...
    roleSelect.innerHTML = ACCOUNT_ROLES[accountType].map(role => `
        <option value="${role.value}">${role.label}</option>
    `).join('');

    roleWrapper.style.display = 'block';
    roleWrapper.querySelector('label').textContent = accountType === 'parent' ? 'Relationship to Child *' : 'Role *';

    form.setAttribute('data-account-type', accountType);

    resetAccountModalFeedback();
}

function resetAccountModalFeedback() {
    const feedback = document.getElementById('account-modal-feedback');
    if (feedback) {
...
function clearAccountModalErrors() {
    const feedback = document.getElementById('account-modal-feedback');
    if (feedback) {
        feedback.textContent = '';
...
function resetAccountModal() {
    ...
    if (form) {
        form.reset();
        form.hidden = false;
        form.setAttribute('data-account-type', 'parent');
    }
    if (successView) {
        successView.hidden = true;
    }
    resetAccountModalFeedback();
    accountModalState.type = 'parent';
}

function handleAccountModalSubmit(event) {
    event.preventDefault();
    const form = event.target;
    const accountType = form.getAttribute('data-account-type') || 'parent';
    ...
    try {
        validateAccountRequest(accountType, formData);
        submitAccountRequest(accountType, formData);
        showAccountModalSuccess(accountType, formData);
    } catch (error) {
        const feedback = document.getElementById('account-modal-feedback');
        if (feedback) {
...
function validateAccountRequest(accountType, data) {
    ...
    if ((accountType === 'parent' || accountType === 'teacher') && !data.classId) {
        throw new Error('Please select a classroom.');
    }

    if (accountType === 'parent' && !data.child) {
        throw new Error('Please include the child name.');
    }
...
function submitAccountRequest(accountType, data) {
    const request = {
        id: `request-${Date.now()}`,
        type: accountType,
    ...
    hydrateAccountRequestsList();
}

const accountRequests = JSON.parse(localStorage.getItem('bccAccountRequests') || '[]');

function showAccountModalSuccess(accountType, data) {
    const form = document.getElementById('account-modal-form');
    const successView = document.getElementById('account-modal-success');
    const successTitle = document.getElementById('account-modal-success-title');
    const successMessage = document.getElementById('account-modal-success-message');

    if (!form || !successView || !successTitle || !successMessage) {
        resetAccountModal();
        closeAccountModal();
        showToast('Request submitted! Our team will follow up shortly.');
        return;
    }

    const copy = ACCOUNT_SUCCESS_COPY[accountType] || ACCOUNT_SUCCESS_COPY.parent;
    accountModalState.type = accountType;

    successTitle.textContent = copy.title;
    successMessage.textContent = copy.message.replace('{name}', data.name || 'there');
...
function persistAccountRequests() {
    try {
        localStorage.setItem('bccAccountRequests', JSON.stringify(accountRequests));
    } catch (error) {
        console.warn('Unable to persist account requests.', error);
    }
}

function hydrateAccountRequestsList() {
    const container = document.getElementById('admin-request-list');
    if (!container) return;

    if (!accountRequests.length) {
        container.innerHTML = '<li class="empty">No pending requests. New submissions will appear here.</li>';
        return;
    }

    container.innerHTML = accountRequests.slice(-5).reverse().map(request => {
        const submitted = new Date(request.submittedAt).toLocaleString();
        const summary = request.type === 'parent'
            ? `${request.data.name} for ${request.data.child || 'Child'} (${getClassById(request.data.classId)?.displayName || 'Class'})`
            : `${request.data.name} • ${ACCOUNT_ROLES[request.type].find(role => role.value === request.data.role)?.label || 'Role'}`;

        return `
            <li>
                <div>
                    <strong>${request.type.charAt(0).toUpperCase() + request.type.slice(1)} request</strong>
                    <p>${summary}</p>
                </div>
                <span>${submitted}</span>
            </li>
...
if (accountModalForm) {
    accountModalForm.addEventListener('submit', handleAccountModalSubmit);
}
